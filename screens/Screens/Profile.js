...
function SPProfile({ navigation }) {
  ...
  const handleDeleteAccount = async () => {
    ...
    if(!password){
      Alert.alert("Password Required","Please Make sure to enter your password")
      return;
    }

    if(password){
      try {
        const { user } = await signInWithEmailAndPassword(AUTH, userData.email, password);
  
        if (user) {
          const user = AUTH.currentUser;
          Alert.alert(
            "Confirmation",
            "Are you sure you want to delete? This will delete all your data.",
            [
              {
                text: "Cancel",
                style: "cancel",
              },
              {
                ...
                onPress: async () => {
                  try {
                    ...
                    Alert.alert("Account Deleted","We hope to see you soon!")
        ...
        Alert.alert("Incorrect Password", "Please enter the correct password to delete your account.");
      }
    }
      ...
      <CurveView style={{ paddingHorizontal: 20 }}>
        <View style={styles.body}>
          <View style={{ alignItems: "center" }}>
            ...
            <Space space={15} />
            <Typo l>Hi,{userData.userName}</Typo>
            <Space space={5} />
          ...
          <DetailCard
            handlPress={() => navigation.navigate("PersonalInfoScreen")}
            icon="user"
            title={"Personal Info"}
          />
          <DetailCard
            handlPress={() => navigation.navigate("MyLikedItems")}
            icon="heart"
            title={"My Liked Items"}
          />
          <DetailCard
            handlPress={() => navigation.navigate("MyAddressScreen")}
            icon="home"
          ...
          <DetailCard
            handlPress={() => setShowDel(true)}
            icon="delete"
            title={"Delete Account"}
          />
          {showDel ? (
            <View>
              <Typo>Password Required to Delete Account.</Typo>
              <InputBox
                value={password}
